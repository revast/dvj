...
#ifdef	LGL_LINUX
//NAR Who exactly _can_ use lgl_glSreenify3D() ?
	/*
	gluLookAt
	(
...
)
{
	...
	LGL.AVCodecSemaphore->Lock("videoEncoderThread","LGL_VideoEncoder::LGL_VideoEncoder() (dst) (meh)");
	{
		...
		strcpy(DstFormatContext->filename, dst);
...
LGL_GetFont()
{
	if(LGL.Font==NULL)
	{
		...
		{
			printf("LGL_GetFont(): Error! Couldn't find default font directory\n");
...
#else	//LGL_LINUX
	// If it's not implemented, don't spin the cycles saying so.
